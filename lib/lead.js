...
function autoTextBody(lead) {
  ...
  return process.env.AUTO_TEXT_TEMPLATE
    ...
    : `Hi ${first}, this is Liquid Build. You're on our waitlist for your ${String(lead.project || 'project').toLowerCase()}. We'll call you to plan it, and construction contracts start once our license transfer is complete. Need a permit sooner? liquidpermit.com can help now. Reply STOP to opt out.`;
}
...
function config() {
  return { license: process.env.LICENSE_NUMBER || '', licenseSince: process.env.LICENSE_NUMBER ? (process.env.LICENSE_SINCE || '') : '', uploads: uploads.ready(), chat: Boolean(process.env.ANTHROPIC_API_KEY) };
}
