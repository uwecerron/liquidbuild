...
async function alertOwner(lead) {
  ...
  const text = [
    ...
    `Location: ${lead.location || '-'}`, ...(lead.files && lead.files.length ? [`Files: ${lead.files.length} attached (open in the CRM)`] : []), `Source: ${lead.source || 'website'}${lead.page ? ' ' + lead.page : ''}`, '', lead.details || '(no details)',
